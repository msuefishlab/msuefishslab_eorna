...
my $db_dir = '/var/www/html/barleyrtd-new/blast_dbs';
...
my %dbs = ( "150831_barley_pseudomolecules" => "150831_barley_pseudomolecules.fasta",
...
my %db_title = ( "150831_barley_pseudomolecules" => "Barley MTP Morex Assembly",
...
my %db_type = ( "150831_barley_pseudomolecules" => "n",
...
eorna::printHeader("BLAST Search Results");
...
if ($number_of_seqs > 30){
  ...
  die ("blast.cgi: Too many sequences submitted\n");
...
sub create_linked_html{
  ...
  foreach my $line(@linked_html){
    ...
    if ($line =~ /><a name=/){
      ...
      $line = "$poo</a><a name = $target_name></a><b><a href='/eorna/search_ids.cgi?seq_name=$m_name&dataset=150831_barley_pseudomolecules'>$m_name</a></b> <a href='javascript:history.go(-1)'>Go Back</a>\n";
...
############################# Make image of the BLAST hits ############################################
  ...
  my $text_offset = 170; # Leave space for the names of the contigs on the left hand side
  ...
  foreach my $q_name(sort keys %blast_results){
    ...
    foreach my $overall_e(sort {$b <=> $a}keys %{$blast_results{$q_name}}){
      foreach my $member(keys %{$blast_results{$q_name}{$overall_e}}){
        ...
        my $link_to_sequence = "transcript.cgi?seq_name=$member&dataset=150831_barley_pseudomolecules";

        $imap .= "<area href='$link_to_sequence' coords='5, $y, $text_offset, $y_end'>\n";
        ...
        $image->string(gdSmallFont, 150 ,$text_height, "$dir", $black);
  ...
  open(PNG, ">/var/www/html/barleyrtd-new/blast_net_images/$imagefile");
  binmode PNG;
  print PNG $image->png();
  close PNG;
